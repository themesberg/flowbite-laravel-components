<nav {{ $attributes->merge(['class' => $navClasses]) }}>
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4 {{ $fluid ? '' : 'container' }}">
        ...
        <div class="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            {{ $actions ?? '' }}
            <button data-collapse-toggle="navbar-{{ $id }}" type="button" class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-{{ $id }}" aria-expanded="false">
