<li class="inline-flex items-center">
    @if($home)
        @if($href)
            <a href="{{ $href }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white" {{ $attributes }}>
    ...
    @else
        <div class="flex items-center">
            ...
            @if($href)
                <a href="{{ $href }}" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white" {{ $attributes }}>{{ $slot }}</a>
            @else
                <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400" {{ $attributes }}>{{ $slot }}</span>
            @endif
        </div>
    @endif
</li>
